...
public class ListInterfaceController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        action.setCellFactory(param -> new TableCell<User, User>() {
            ...
            private final Button opinionsButton = new Button("Voir avis");
            ...
            protected void updateItem(User user, boolean empty) {
                ...
                deleteButton.setOnAction( //event -> getTableView().getItems().remove(user)
                        event -> {
                            us.supprimerPersonne(user);

                               showAlert(Alert.AlertType.INFORMATION, ((Node) event.getSource()).getScene().getWindow(),
                                           "succès!", "suppression d'utilisateur avec succès");
                            getTableView().getItems().remove(user);

                        }
                );
                
                editButton.setOnAction( //event -> getTableView().getItems().remove(user)
                        event -> {
                            try {
